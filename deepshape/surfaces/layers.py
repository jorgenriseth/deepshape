...
from torch import sin, cos, arange, randn, zeros
from torch.nn import Parameter, Upsample

from ..common.LayerBase import SurfaceLayer
...
class SineLayer(SurfaceLayer):
    ...
    def derivative(self, x, h=None):
        ...
        self.D[:, 0, 0, :n] = self.nvec * pi * \
            C1[:, 0, :]  # Type 1 x direction dx
        self.D[:, 1, 1, N:(N+n)] = self.nvec * pi * \
            C1[:, 1, :]  # Type 1  y-direction dy

        self.D[:, 0, 0, n:(n + n**2)] = T11[:, 0, :]  # Type 2 x-direction dx
        self.D[:, 0, 1, n:(n + n**2)] = T12[:, 0, :]  # Type 2 x-direction dy
        self.D[:, 1, 1, (N+n):(N + n + n**2)] = T11[:,
                                                    1, :]  # Type 2 y-direction dy
        self.D[:, 1, 0, (N+n):(N + n + n**2)] = T12[:,
                                                    1, :]  # Type 2 x-direction dy
        ...
        return torch.eye(2, 2) + self.D @ self.weights
